...
import { downloadCommand, downloadCallback, downloadRefresh, handlePendingDownloadEdit, tryDownloadFromText } from './commands/download.js';
...
// --- Free-text: pending download edit → Stremio/magnet/hash → Claude ---
bot.on('text', async (ctx, next) => {
  if (handlePendingDownloadEdit(ctx)) return;
  if (await tryDownloadFromText(ctx)) return;
  return next();
});
