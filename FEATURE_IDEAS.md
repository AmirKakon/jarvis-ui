...
14. **Israel buses — live times, near-stop alerts, NL Q&A** — split by surface so ambient UI stays cheap and chat stays flexible:
    ...
    - **JARVIS Q&A (implemented):** `agents/bus.js` + front-router `{"bus": true}` + `/bus` — answers “when’s the next bus / 65 / 616?” by reading those HA sensors (same data as the dashboard). Works on Telegram and Assist→JARVIS (`/v1/chat/completions`) once Assist is pointed at the bot.
